#include <realme.auth/realme.auth.hpp>
...
namespace amax {
   ...
   void amax_auth::init( const name& realme_dao, const name& realme_owner) {
      CHECKC(has_auth(_self),  err::NO_AUTH, "no auth for operate");

      _gstate.amax_dao_contract    = realme_dao;
      _gstate.realme_owner_contract      = realme_owner;
   }
   ...
   void amax_auth::newaccount(const name& auth, const name& creator, const name& account, const string& info, const authority& active) {
      ...
      realme_dao::newaccount_action newaccount_act(_gstate.amax_dao_contract, { {get_self(), ACTIVE_PERM} });
   ...
   void amax_auth::bindinfo ( const name& auth, const name& account, const string& info) {
      ...
      realme_dao::checkauth_action checkauth_act(_gstate.amax_dao_contract, { {get_self(), ACTIVE_PERM} });
