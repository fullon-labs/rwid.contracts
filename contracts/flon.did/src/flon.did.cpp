...
#define CHECKC(exp, code, msg) \
   ...
   void flon_did::setdidstatus( const uint64_t& order_id, const name& status, const string& msg ) {
   ...
   void flon_did::_add_pending( const uint64_t& order_id ) {
   ...
   void flon_did::_del_pending( const uint64_t& order_id ) {
      pending_t::idx_t pendings(_self, _self.value);
      auto pending_ptr     = pendings.find(order_id);
   ...
   void flon_did::addvendor(const string& vendor_name, const name& vendor_account,
                        uint32_t& kyc_level,
   ...
   void flon_did::chgvendor(const uint64_t& vendor_id, const name& status,
                           const asset& user_reward_quant,
                           const asset& user_charge_quant, 
                           const nsymbol& nft_id) {
   ...
   void flon_did::auditlog( 
    ...
    void flon_did::_on_audit_log(
            ...
            flon_did::auditlog_action act{ _self, { {_self, active_permission} } };
