...
namespace eosiosystem {
  ...
  /**
   * The `flon.system` smart contract is provided by `Armoniax` as a sample system contract, and it defines the structures and actions needed for blockchain's core functionality.
   * 
   * Just like in the `flon.bios` sample contract implementation, there are a few actions which are not implemented at the contract level (`newaccount`, `updateauth`, `deleteauth`, `linkauth`, `unlinkauth`, `canceldelay`, `onerror`, `setabi`, `setcode`), they are just declared in the contract so they will show in the contract's ABI and users will be able to push those actions to the chain via the account holding the `flon.system` contract, but the implementation is at the flon core level. They are referred to as flon native actions.
   * 
   ...
   */
   ...
   struct [[eosio::table("global"), eosio::contract("flon.system")]] flon_global_state : eosio::blockchain_parameters {
      ...
      EOSLIB_SERIALIZE_DERIVED( flon_global_state, eosio::blockchain_parameters,
   ...
   typedef eosio::singleton< "global"_n, flon_global_state >   global_state_singleton;
   ...
   class [[eosio::contract("flon.system")]] system_contract : public native {
      ...
      public:
         ...
         static constexpr eosio::name reserv_account{"flon.reserv"_n};
      ...
      private:
         ...
         static flon_global_state get_default_parameters();
