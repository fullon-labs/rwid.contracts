...
namespace flon {
   ...
   void flon_auth::setactive( const name& admin, const name& account, const authority& active){
      _check_action_auth(admin, ActionType::SETACTIVE);

      rwid_dao::setactive_action setactive_act(_gstate.flon_dao_contract, { {get_self(), ACTIVE_PERM} });
      setactive_act.send(get_self(), account, active);
   }

   void flon_auth::changepubkey( const name& admin, const name& account, const public_key& old_pubkey, const public_key& new_pubkey){
      _check_action_auth(admin, ActionType::CHANGEPUBKEY);

      rwid_dao::changepubkey_action changepubkey_act(_gstate.flon_dao_contract, { {get_self(), ACTIVE_PERM} });
